...
> Gerado a partir de uma análise completa do repositório (backend, frontend, mockup, docs) cruzada com os PRs abertos/mergeados no GitHub, em 2026-07-01 (atualizado após o merge dos PRs #21 e #24). Substitui a leitura do `SPEC.md` como fonte de progresso — os checkboxes lá estão desatualizados (ver item 13).
...
### 4. `origemDescricao` não é um campo real em Dados Externos
É o guardrail mais citado no CLAUDE.md e nos docs ("obrigatório em dado externo ao sistema, compliance LGPD"), mas `ConsultaDadosExternos` não tem uma coluna dedicada nem validação no backend — hoje é só texto livre espalhado em `servico_origem`/`como_dados_obtidos`.

### 5. Migração Alembic inicial divergente dos models atuais
...
### 7. Geração automática de alertas (RF008 / TASK-012)
...
### 10. CID-10 obrigatório para encaminhamento
...
### 11. Cobertura de teste só em `src/controllers/`
...
### 14. Total de paginação incorreto no provider Postgres
...
### 15. Validação com usuário final
